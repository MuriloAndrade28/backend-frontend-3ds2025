...
const API_BASE = "http://localhost/3ds-exemplo/backend/api_aluno.php";
...
export default function Alunos() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, padding: 16 }}>
      <Text style={{ fontSize: 22, fontWeight: "700" }}>Alunos</Text>
      ...
      {Array.isArray(data) && (
        <FlatList
          style={{ marginTop: 12 }}
          data={data}
          keyExtractor={(item) => String(item.alu_id)}
          refreshControl={
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              <Text>Email: {item.alu_email}</Text>
              <Text>ID: {item.alu_id}</Text>
            </View>
          )}
        />
      )}
